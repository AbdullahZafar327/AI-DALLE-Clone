import React from 'react'

import {download} from '../assets';
import {downloadImage} from '../utils'

const Card = ({_id,name,prompt,photo}) => {
  return (
    <div className='rounded-xl group relative card shadow-card hover:shadow-cardhover'>
      <img
      className='w-full h-auto object-contain rounded-xl'
      src={photo}
      alt={prompt}
      />
      <div className='group-hover:flex flex-col absolute max-h-[94.5%] hidden bottom-0 right-0 left-0 bg-[#10131f] m-2 p-4 rounded-md'>
        <p className='text-white text-sm prompt overflow-y-auto'>{prompt}</p>
        <div className='flex justify-between mt-3 items-center gap-2'>
          <div className='flex items-center gap-2'>
             <div className='h-7 w-7 rounded-full bg-green-700 flex object-cover justify-center items-center text-white text-xs font-bold'>
              {name[0]}
             </div>
             <p className='text-white text-sm'>{name}</p>
          </div>
          <button type="button" onClick={()=>downloadImage(_id,photo)} className='outline-none bg-transparent border-none'>
               <img src={download} alt="download" className='h-6 w-6 object-contain invert'/>
          </button>
        </div>
      </div>
    </div>
  )
}

export default Card
